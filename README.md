...
| <a name="input_crds_argo_name"></a> [crds\_argo\_name](#input\_crds\_argo\_name) | Name of the ArgoCD Application. Required if `crds_argo_source_type` is set to `kustomize` or `directory`.  If `crds_argo_source_type` is set to `helm`, ArgoCD Application name will equal `helm_release_name`. Defaults to `""`. | `string` |
